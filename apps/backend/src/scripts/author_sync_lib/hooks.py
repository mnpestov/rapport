...
def _insales_amount(el):
    # data-product-price — ЕДИНСТВЕННЫЙ серверный источник числа, а не просто
    # более удобный: проверено на anastasiiastupa.ru — в отдаваемом сервером
    # HTML сам span ПУСТ (<span ... data-product-price="1700.0"></span>),
    # цифры дорисовывает JS уже в браузере. Обычный GET+BeautifulSoup видит
    # только "₽ ₽", поэтому разбор видимого текста (как во всех остальных
    # механизмах этой цепочки) на InSales вернул бы None у каждого товара.
    # Текст оставлен запасным путём на случай темы, которая рендерит число
    # сервером: _parse_woo_price умеет и неразрывный пробел как разделитель
    # тысяч ("1\xa0700"), и приклеенный символ валюты.
...
def _extract_insales_price(soup):
    ...
    # скидка: <strike> с пустым span'ом внутри отдаётся на каждом товаре.
    # Это ровно тот случай, про который предупреждает
    # DETAILS_PRICE_PARSING_PROCESS.md ("скрытые/задвоенные элементы").
    # Отсюда две независимые проверки, и порядок их важности НЕ такой, каким
    # кажется по разметке из DevTools:
    #
    #   (1) число вообще разобралось и оно БОЛЬШЕ текущей цены. Это рабочая
    #       защита: без скидки span пуст и data-атрибута на нём нет, значит
    #       _insales_amount вернёт None. Условие "больше текущей" закрывает
    #       тему, которая оставила бы в <strike> протухшее значение —
    #       "старая" цена ниже или равная текущей это не скидка, а мусор
    #       (на фронте скидка считается как oldPrice > price).
    #   (2) display:none / hidden на самом элементе или на предках. В
    #       DevTools у <strike> действительно висит style="display: none",
    #       но проверено на живых страницах: в СЕРВЕРНОМ HTML атрибута нет
    #       вовсе — его дописывает JS при гидратации. Скрапер работает с
    #       сырым ответом, поэтому здесь эта ветка не срабатывает никогда.
    #       Оставлена как страховка на случай темы, прячущей <strike>
    #       сервером, и на случай разбора уже гидратированного DOM.
