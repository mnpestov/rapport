...
//
// 3 запроса в минуту. Изначально стоял 1/мин, но этого мало: человек
// подписывается на канал и жмёт «Проверить подписку» несколько раз подряд —
// Telegram отдаёт членство не мгновенно. Со строгим лимитом вторая попытка
// упиралась в 429. Три запроса покрывают такой сценарий и по-прежнему
// защищают шлюз от амплификации.
